...
namespace AudioData
{
    internal class HammingEncoder
    {
        ...
        public static bool[] Encode(bool[] code)
        {
            ...
            { 
                if(i % 8 == 0)
                {
                    List<bool> toEncode = new List<bool>();
                    for (int j = i; j < 8; j++)
                    {
                        toEncode.Add(code[j]);
                    } // Take the first 8 bools and place them in the list.

                    toEncode = HEncode(toEncode.ToArray()).ToList(); 

                    foreach (bool b in toEncode)
                    {
                        result.Add(b);
                    }
                }
        ...
        public static bool[] Decode(bool[] code)
        {
            ...
            //        toEncode = HDecode(toEncode.ToArray()).ToList();
    ...
    /// <summary>
    /// I did not write this class. Idk who the original author is.
    /// </summary>
